...
def _usage_live_status_refresh_check(swift_source: Path) -> Check:
    ...
    required = {
        ...
        "recent live merge": "mergeUsageEvents(readRecentEvents(dataRoot: dataRoot, since: recentCutoff), with: liveRecentEvents)",
        "fast live status source": "let statusEvents = recentEvents",
        "running agent state": "let runningActivity = runningAgentActivity()",
